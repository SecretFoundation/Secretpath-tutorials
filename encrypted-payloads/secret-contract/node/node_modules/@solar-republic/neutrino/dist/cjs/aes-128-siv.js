...
const split_siv_key = async (atu8_key) => {
    if (32 !== atu8_key.byteLength)
        die('SIV key not 32 bytes');
    // destructure halves
...
export const aes_128_siv_encrypt = async (atu8_key, atu8_plaintext, a_ad = [ATU8_NIL]) => {
    // construct aes keys
 ...
 */
// eslint-disable-next-line @typescript-eslint/naming-convention
export const aes_128_siv_decrypt = async (atu8_key, atu8_payload, a_ad = [ATU8_NIL]) => {
    const [d_key_cbc, d_key_ctr] = await split_siv_key(atu8_key);
    if (atu8_payload.byteLength < NB_AES_BLOCK)
        die(`SIV payload < ${NB_AES_BLOCK} bytes`);
    ...
    if (atu8_cmac.length !== NB_AES_BLOCK || atu8_tag.length !== NB_AES_BLOCK)
        die(`Invalid tag/CMAC lengths`);
...
//# sourceMappingURL=aes-128-siv.js.map