...
export const retry = async (f_broadcast, f_handle, c_attempts = 0) => {
    try {
 ...
 */
const without_throwing = (dp_req) => new Promise(fk_resolve => dp_req.then(fk_resolve, fk_resolve));
/**
 ...
 */
export const subscribe_tendermint_events = (p_rpc, sx_query, fk_message) => new Promise((fk_resolve, fe_reject) => oda(
...
new WebSocket('ws' + p_rpc.slice(4) + '/websocket'), {
    ...
    onmessage(g_msg) {
        // parse message
        const g_data = safe_json(g_msg.data);
        // expect confirmation
 ...
 */
export const broadcast_result = async (gc_node, atu8_raw, si_txn) => new Promise(async (fk_resolve, fe_reject) => {
    // prep websocket
    let d_ws;
    // prep periodic query
    let i_periodic = 0;
    ...
    try {
        [d_ws] = await timeout_exec(30e3, () => subscribe_tendermint_events(gc_node.rpc, `tx.hash='${si_txn}'`, (d_event) => {
            ...
            f_shutdown();
            // parse message frame
            const g_tx_res = safe_json(d_event.data)?.result.data.value.TxResult;
...
export const query_secret_contract = async (k_contract, h_query) => {
    ...
    // not successful
        ...
        if (e_query[0]) {
            // destructure details
            const [d_res, s_res, g_res] = e_query;
            // destructure chain code
            const xc_code = g_res?.code;
            // error message
            const m_error = /encrypted: (.+?):/.exec(g_res?.message || '');
 ...
 * @returns
 */
export const format_secret_query = (si_method, h_query, z_auth) => (z_auth
    ? z_auth.at
        // array?
        ? z_auth.map
            // ViewerInfo
 ...
 *  - [3]: `h_answer?: JsonObject` - contract response as JSON object on success
 */
export const query_secret_contract_infer = async (k_contract, si_method, ...[h_args, z_auth]) => {
 ...
 */
export const exec_secret_contract = async (k_contract, k_wallet, h_exec, a_fees, sg_limit, sa_granter, a_funds, s_memo) => {
    // prep plaintext
    let s_plaintext;
    ...
    if (import.meta.env?.DEV) {
        ...
        if (g_tx_res) {
            const { gas_used: sg_used, gas_wanted: sg_wanted, } = g_tx_res.result;
 ...
 * @returns
 */
export const sign_secret_query_permit = async (k_wallet, si_permit, a_tokens, a_permissions) => {
    // prep params
    const g_params = {
    ...
    // sign query permit
    const [atu8_signature, g_signed] = await sign_amino(k_wallet, [{
            type: 'query_permit',
            value: g_params,
        }], [['0', 'uscrt']], '1', ['0', '0']);
    // encode query permit
...
//# sourceMappingURL=app-layer.js.map