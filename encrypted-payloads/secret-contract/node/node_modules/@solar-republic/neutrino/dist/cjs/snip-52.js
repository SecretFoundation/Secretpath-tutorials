 ...
 * @param h_channels
 */
export const subscribe_snip52_channels = async (p_rpc, k_contract, z_auth, h_channels) => {
    const h_resolved = {};
    // fetch channel info for all requested channels at once
    let [g_result, xc_code, s_error] = await query_secret_contract_infer(k_contract, 'channel_info', {
    ...
    for (const g_channel of g_result.channels) {
        const si_channel = g_channel.channel;
        // parse seed
        ...
        if ('counter' === g_channel.mode) {
            ...
            if (si_notification !== g_channel.next_id)
                die('Failed to derive accurate notification ID');
    ...
    const d_ws = await subscribe_tendermint_events(p_rpc, `wasm.contract_address='${k_contract.addr}'`, async (d_event) => {
        // parse message frame
        const g_jsonrpc_result = safe_json(d_event.data).result;
        let h_events = g_jsonrpc_result.events;
        // check each channel
        for (let si_notification in h_resolved) {
            let [si_channel, atu8_seed, xg_counter, xg_hash, next_id, fk_notification] = h_resolved[si_notification];
            ...
            if (a_received) {
                ...
                fk_notification(cbor_decode_trivial(atu8_message)[0]);
...
export const sign_seed_update = async (k_wallet, sa_contract, sb64_previous) => {
    // prep params
    const g_params = {
...
//# sourceMappingURL=snip-52.js.map