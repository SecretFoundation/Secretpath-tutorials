...
// import aes key
export const aes_key = (atu8_key, si_algo) => crypto.subtle.importKey('raw', atu8_key, si_algo, false, ['encrypt']);
// perform AES-CBC
export const aes_cbc = async (d_key_cbc, atu8_data) => bytes(await crypto.subtle.encrypt({
    name: 'AES-CBC',
    iv: ATU8_ZERO_BLOCK.slice(),
}, d_key_cbc, atu8_data), 0, NB_AES_BLOCK);
// perform AES-CTR
export const aes_ctr = async (d_key_ctr, atu8_iv, atu8_data) => bytes(await crypto.subtle.encrypt({
    name: 'AES-CTR',
    counter: atu8_iv,
    length: NB_AES_BLOCK,
}, d_key_ctr, atu8_data));
// pseudo-constant-time select
const select = (xb_value, xb_rif1, xb_rif0) => (~(xb_value - 1) & xb_rif1) | ((xb_value - 1) & xb_rif0);
// double block value in-placce
const double_block = (atu8_block) => {
    let xb_carry = 0;
...
export const s2v = async (d_key_rkd, atu8_plaintext, a_ad = [ATU8_NIL]) => {
