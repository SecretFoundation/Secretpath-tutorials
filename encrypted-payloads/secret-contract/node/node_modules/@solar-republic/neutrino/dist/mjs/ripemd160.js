...
/* eslint-enable */
const twid = (i_group, xn_x, xn_y, xn_z) => {
    if (0 === i_group)
        return xn_x ^ xn_y ^ xn_z;
    else if (1 === i_group)
        return (xn_x & xn_y) | (~xn_x & xn_z);
    else if (2 === i_group)
        return (xn_x | ~xn_y) ^ xn_z;
    else if (3 === i_group)
        return (xn_x & xn_z) | (xn_y & ~xn_z);
    else
        return xn_x ^ (xn_y | ~xn_z);
};
/* eslint-disable prefer-const */
export const ripemd160 = (atu8_data) => {
    init();
    /* eslint-disable @typescript-eslint/naming-convention */
    ...
    let ib_write = 0;
    const f_round = (dv_read, ib_offset) => {
        ...
        for (let i_group = 0; i_group < 5; i_group++) {
            let [xn_hbl, xn_hbr, atu8_rl, atu8_rr, atu8_sl, atu8_sr,] = [
                ATU8_KL, ATU8_KR,
                A_INDEXES_L, A_INDEXES_R,
                A_SHIFTS_L, A_SHIFTS_R,
            ].map(atu8 => atu8[i_group]);
