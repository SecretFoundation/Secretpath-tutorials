...
const NB_FIELD = 32;
const crv = (xg_x) => mod((mod(xg_x * xg_x) * xg_x) + 7n);
const is_field_element = (xg_value) => xg_value > 0n && xg_value < XG_FIELD_PRIME;
const is_group_element = (xg_value) => xg_value > 0n && xg_value < XG_CURVE_ORDER;
const exceeds_half_order = (xg_n) => xg_n > (XG_CURVE_ORDER >> 1n);
const extract_bigint_from_buffer = (atu8, ib_lo, ib_hi) => bytes_to_bigint_be(atu8.subarray(ib_lo, ib_hi));
const mod = (xg_value, xg_mod = XG_FIELD_PRIME) => {
...
const sqrt = (xg_n) => {
    ...
    for (let xg_value = xg_n, xg_e = (XG_FIELD_PRIME + 1n) / 4n; xg_e > 0n; xg_e >>= 1n) {
        if (xg_e & 1n)
            xg_r = (xg_r * xg_value) % XG_FIELD_PRIME;
...
// };
const ec_point = ([xg_x, xg_y, xg_z]) => ({
    a: [xg_x, xg_y, xg_z],
    eq(k_other) {
        const [xg_x2, xg_y2, xg_z2] = k_other.a;
        return mod(xg_x * xg_z2) === mod(xg_x2 * xg_z) && mod(xg_y * xg_z2) === mod(xg_y2 * xg_z);
    },
    ng: () => ec_point([xg_x, mod(-xg_y), xg_z]),
    add(k_other) {
        const [xg_x2, xg_y2, xg_z2] = k_other.a;
    ...
    mul(xg_n, xc_safe = 1) {
        if (!xc_safe && 0n === xg_n)
            return KP_ZERO;
        if (!is_group_element(xg_n))
            die('Invalid scalar');
        // if(this.eq(KP_BASE)) return pre_base_mul(xg_n).p;
        let kp_p = KP_ZERO;
        let kp_f = KP_BASE;
        for (let kp_d = this; xg_n > 0n; kp_d = kp_d.add(kp_d), xg_n >>= 1n) {
            if (xg_n & 1n) {
                kp_p = kp_p.add(kp_d);
            }
            else if (xc_safe) {
...
const import_ec_point = (atu8_data) => {
    let yp_new;
    ...
    if (!yp_new)
        return die('Invalid point data');
    return yp_new.ok();
};
const invert = (xg_value, xg_md = XG_FIELD_PRIME) => {
    if (0n === xg_value || xg_md <= 0n)
        die('No inverse');
...
const normalize_sk = (z_sk) => {
    if ('bigint' !== typeof z_sk)
        z_sk = bytes_to_bigint_be(z_sk);
    return is_group_element(z_sk) ? z_sk : die('Invalid private key');
};
const bitsequence_to_uint = (atu8_data) => {
...
const hmac_drbg = async (atu8_seed_root, f_predicate) => {
    ...
    const f_reseed = async (atu8_seed) => {
...
export const gen_sk = () => ent_to_sk(crypto.getRandomValues(bytes(NB_FIELD + 8)));
export const ent_to_sk = (atu8_entropy) => atu8_entropy.length < NB_FIELD + 8 || atu8_entropy.length > 1024
    ? die('Invalid entropy')
    : bigint_to_bytes_be(mod(bytes_to_bigint_be(atu8_entropy), XG_CURVE_ORDER - 1n) + 1n);
export const sk_to_pk = (z_sk, xc_uncompressed = 0) => KP_BASE.mul(normalize_sk(z_sk)).out(xc_uncompressed);
export const ecdh = (atu8_sk, atu8_pk, b_uncompressed) => import_ec_point(atu8_pk).mul(normalize_sk(atu8_sk)).out(b_uncompressed || 0);
export const sign = async (atu8_sk, atu8_hash, atu8_ent) => {
    ...
    return hmac_drbg(atu8_seed, (atu8_k) => {
        const xg_k = bitsequence_to_uint(atu8_k);
        if (!is_group_element(xg_k))
            return;
        ...
        let xc_recovery = ((a_q[0] === xg_r ? 0 : 2) | Number(a_q[1] & 1n));
        ...
        return [atu8_out, xc_recovery];
    });
};
export const verify = (atu8_signature, atu8_hash, atu8_pk, b_low_s = true) => {
    if (2 * NB_FIELD !== atu8_signature.length)
    ...
    try {
        ...
        if (!is_group_element(xg_r) || !is_group_element(xg_s))
            return false;
    ...
    if (b_low_s && exceeds_half_order(xg_s))
        return false;
    let a_aff_r;
    try {
...
const hmac_sha256 = async (atu8_key, atu8_data) => {
