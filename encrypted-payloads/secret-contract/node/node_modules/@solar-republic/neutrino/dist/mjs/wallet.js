...
import { sign, sk_to_pk } from './secp256k1.js';
 ...
 */
export const pubkey_to_bech32 = async (atu8_pk_33, s_hrp = 'secret') => {
...
export const Wallet = async (atu8_sk, si_chain, p_lcd, p_rpc, s_hrp = si_chain.replace(/-.*/, '')) => {
    // obtain public key
    ...
    return {
        ...
        addr: sa_account,
        pk33: atu8_pk33,
        sign: (atu8_hash, atu8_k = random_32()) => sign(atu8_sk, atu8_hash, atu8_k),
    };
};
/**
 ...
 */
// eslint-disable-next-line @typescript-eslint/naming-convention
export const auth = async (g_wallet, a_auth) => {
    // resolve auth data
    if (!a_auth) {
        let sg_account;
        let sg_sequence;
 ...
 */
export const sign_amino = async (k_wallet, a_msgs, // eslint-disable-line @typescript-eslint/naming-convention
a_fees, sg_limit, a_auth, // eslint-disable-line @typescript-eslint/naming-convention
s_memo, sa_granter, sa_payer) => {
    // resolve auth data
    ...
    // prepare message
    const atu8_signdoc = text_to_bytes(JSON.stringify(g_signdoc)
        .replace(/&/g, '\\u0026')
        .replace(/</g, '\\u003c')
        .replace(/>/g, '\\u003e'));
    // hash message
 ...
 * @returns
 */
export const sign_direct = async (k_wallet, atu8_auth, atu8_body, sg_account) => {
    // encode signdoc
 ...
 * @returns
 */
export const create_tx_body = async (xc_sign_mode, k_wallet, a_msgs, a_fees, sg_limit, a_auth, // eslint-disable-line @typescript-eslint/naming-convention
s_memo, sa_granter, sa_payer) => {
    // resolve auth data
    const [sg_account, sg_sequence] = await auth(k_wallet, a_auth);
    // encode pubkey
    const atu8_pubkey = any('/cosmos.crypto.secp256k1.PubKey', encodeCosmosCryptoSecp256k1PubKey(k_wallet.pk33));
    // encode signer info
    const atu8_signer = encodeCosmosTxSignerInfo(atu8_pubkey, encodeCosmosTxModeInfo(encodeCosmosTxModeInfoSingle(xc_sign_mode)), sg_sequence);
    // encode fee
 ...
 * @returns
 */
export const create_and_sign_tx_direct = async (k_wallet, a_msgs, a_fees, sg_limit, a_auth, // eslint-disable-line @typescript-eslint/naming-convention
s_memo, sa_granter, sa_payer) => {
    // create tx
    const [atu8_auth, atu8_body, sg_account,] = await create_tx_body(XC_PROTO_COSMOS_TX_SIGNING_SIGN_MODE_DIRECT, k_wallet, a_msgs, a_fees, sg_limit, a_auth, s_memo, sa_granter, sa_payer);
    // sign direct
