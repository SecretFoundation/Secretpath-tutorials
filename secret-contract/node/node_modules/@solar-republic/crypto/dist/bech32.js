...
const regroup_bits = (a_words, ni_in, ni_out, xc_pad = 0, xm_mask = (1 << ni_out) - 1) => {
    const a_out = [];
    let xb_tmp = 0;
    let xi_carry = 0;
    ...
    if (xc_pad && xi_carry)
        a_out.push((xb_tmp << (ni_out - xi_carry)) & xm_mask);
    return a_out;
};
export const pubkey_to_bech32 = (z_pubkey, si_hrp) => {
 ...
 * @returns
 */
export const bech32_encode = (si_hrp, atu8_data) => {
...
// };
// eslint-disable-next-line @typescript-eslint/naming-convention
export const bech32_decode = (sa_bech32) => {
    if (!sa_bech32)
        return __UNDEFINED;
    ...
    for (const s_char of sx_data) {
        ...
        if (x_word < 0)
            die('Unknown character ' + s_char);
        xb_checksum = polymod_step(xb_checksum) ^ x_word;
        // do not add checksum to word data
        if (a_words.length >= sx_data.length - 6)
            continue;
        // add numeric value to word data
        a_words.push(x_word);
    }
    if (1 !== xb_checksum)
        die('Invalid checksum for ' + sa_bech32);
