...
export type SignedAminoDoc<h_config extends JsonObject> = {
    params: h_config & {
        chain_id: string;
    };
    signature: {
...
export interface TypedAminoMsg<si_type extends string = string, g_value extends JsonObject = JsonObject> extends AminoMsg, JsonObject {
    readonly type: si_type;
    readonly value: g_value;
}
export interface TypedStdSignDoc<a_msgs extends readonly TypedAminoMsg[] = TypedAminoMsg[]> extends StdSignDoc, JsonObject<a_msgs> {
 ...
 * JSON-RPC response
 */
export type JsonRpcResponse<w_result extends JsonObject> = {
    jsonrpc: '2.0';
    id: string;
    result: w_result;
};
export type TendermintEvent<w_value extends JsonObject> = {
    query: string;
    data: {
