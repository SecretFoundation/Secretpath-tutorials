...
const ident = (yn) => yn.name;
const litval = (yn) => yn.value;
let si_import_key = '';
let si_init_key = '';
let xc_fd_close = 0;
let xc_fd_seek = 0;
...
const H_RENAME_IMPORTS = {
    _abort: 'abort',
    _emscripten_memcpy_js: 'memcpy',
    _emscripten_resize_heap: 'resize',
    _fd_write: 'write',
};
const H_KNOWN_EXPORT_TYPES = {
    _malloc: '(nb_size: ByteSize) => Pointer',
    _free: '(ip_ptr: Pointer) => void',
    _sbrk: '(nb_change: ByteDelta) => Pointer',
    wasmMemory: 'WebAssembly.Memory',
};
const rename_export = (si_func) => si_func.replace(/^_(?:secp256k1_)?/, '').replace('wasmMemory', 'memory');
const export_type = (si_func) => H_KNOWN_EXPORT_TYPES[si_func] || 'Function';
for (const yn_top of yn_root.body) {
    if ('ExpressionStatement' === yn_top.type) {
        for (const yn_arg of yn_top.expression.arguments) {
            if ('ArrowFunctionExpression' === yn_arg.type) {
                const a_stmts = yn_arg.body.body;
                    ...
                    if ('ExpressionStatement' === yn_stmt.type) {
                        ...
                        if ('AssignmentExpression' === yn_expr.type) {
                            ...
                            if ('MemberExpression' === yn_right.type) {
                                const si_symbol = litval(yn_right.property);
                        ...
                        if ('ObjectExpression' === yn_init?.type) {
                            for (const yn_prop of yn_init.properties) {
                                if ('wasmImports' === ident(yn_prop.value)) {
                                    si_import_key = litval(yn_prop.key);
                                }
                            }
                        }
                    },
            ...
            _fd_seek() {
                for (const yn_stmt of a_body) {
                    if ('ReturnStatement' === yn_stmt.type) {
                        xc_fd_seek = litval(yn_stmt.argument);
                    }
                }
            },
            initRuntime() {
                        ...
                        if ('CallExpression' === yn_call.type) {
                            ...
                            if ('MemberExpression' === yn_callee.type) {
                                if ('wasmExports' === ident(yn_callee.object)) {
                                    si_init_key = litval(yn_callee.property);
                                }
                            }
                        }
                    }
